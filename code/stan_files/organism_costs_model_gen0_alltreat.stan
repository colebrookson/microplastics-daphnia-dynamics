...
functions { // dz_dt holds all state variables (in our case 6)
  ...
  real[] dll_400_dt(real t, 
    ...
    real d_2000_cq = ke*(2000-cq_2000);
    ...
    real d_10000_cq = ke*(10000-cq_10000);
...
model {
  ...
  for(y in 2:22){ // every day from 2 to 21
    ...
    R0[y] = R0[y-1] + eq0[y]*(Rm/(1-(Lp^3)))*
                      (
                        (1*((z_ll_con_temp)^2)) *
                        ((1+z_ll_con_temp)/(1+1))-(Lp^3)
                        )*
                        ((1+s_cq_con)^-1);
                
    R1[y] = R1[y-1] + eq1[y]*(Rm/(1-(Lp^3)))*
                      ((1*((z_ll_400_temp)^2)) *
                      ((1+z_ll_400_temp)/(1+1))-(Lp^3))*
                      ((1+s_cq_400)^-1);
                
    R2[y] = R2[y-1] + eq2[y]*(Rm/(1-(Lp^3)))*
                      ((1*((z_ll_2000_temp)^2)) *
                      ((1+z_ll_2000_temp)/(1+1))-(Lp^3))*
                      ((1+s_cq_2000)^-1);
                
    R3[y] = R3[y-1] + eq3[y]*(Rm/(1-(Lp^3)))*
                      ((1*((z_ll_10000_temp)^2)) *
...
generated quantities {
  ...
  for(y in 2:22){ // every day from 2 to 21
    ...
    // cumulative reproduction at each time step
    R0_rep[y] = R0_rep[y-1] + eq0_rep[y]*(Rm/(1-(Lp^3)))*
                              ((1*((z_ll_con_temp_rep)^2)) *
                              ((1+z_ll_con_temp_rep)/(1+1))-(Lp^3))*
                              ((1+s_cq_con_rep)^-1);
    R1_rep[y] = R1_rep[y-1] + eq1_rep[y]*(Rm/(1-(Lp^3)))*
                              ((1*((z_ll_400_temp_rep)^2)) *
                              ((1+z_ll_400_temp_rep)/(1+1))-(Lp^3))*
                              ((1+s_cq_400_rep)^-1);
    R2_rep[y] = R2_rep[y-1] + eq2_rep[y]*(Rm/(1-(Lp^3)))*((1*
                              ((z_ll_2000_temp_rep)^2)) *
                              ((1+z_ll_2000_temp_rep)/(1+1))-(Lp^3))*
                              ((1+s_cq_2000_rep)^-1);
    R3_rep[y] = R3_rep[y-1] + eq3_rep[y]*(Rm/(1-(Lp^3)))*
                              ((1*((z_ll_10000_temp_rep)^2)) *
                              ((1+z_ll_10000_temp_rep)/(1+1))-(Lp^3))*
                              ((1+s_cq_10000_rep)^-1);
